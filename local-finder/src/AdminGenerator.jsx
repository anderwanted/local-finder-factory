...
`;
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '20px', fontFamily: 'sans-serif', color: '#334155' }}>
      
      {/* HEADER DE INSTRUÇÕES */}
      <div style={{ background: 'white', padding: '25px', borderRadius: '16px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', marginBottom: '25px', borderLeft: `6px solid ${corTema}` }}>
        ...
        <div style={{ marginTop: '15px' }}>
          <button
            ...
            style={{
              ...
              fontWeight: 'bold', fontSize: '16px', cursor: rawInput ? 'pointer' : 'not-allowed',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px',
              transition: 'all 0.2s', boxShadow: rawInput ? '0 4px 12px rgba(0,0,0,0.15)' : 'none'
            }}
          >
            {copied ? <><Check /> Prompt Copiado! Cole na IA.</> : <><Copy /> Gerar Comando SQL</>}
...
// Componente visualzinho dos passos
function StepCard({ icon, title, text, color }) {
  return (
    <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', color: color, fontWeight: 'bold' }}>
        {icon} {title}
      </div>
      <p style={{ margin: 0, fontSize: '12px', color: '#64748b', lineHeight: '1.4' }}>{text}</p>
    </div>
  )
}
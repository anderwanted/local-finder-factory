...
export default function Dashboard({ projeto }) {
  ...
  async function handleUpdateLocal() {
    ...
    if (!error) { setEditingId(null); buscarDados(); }
  }
    ...
    <div style={{ background: theme.bg, minHeight: '100vh', color: theme.text, padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: '20px' }}>
        ...
        <aside style={{ width: '240px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <button onClick={() => setActiveTab('lojas')} style={{ padding: '12px', border: 'none', borderRadius: '8px', textAlign: 'left', cursor: 'pointer', display: 'flex', gap: '10px', background: activeTab === 'lojas' ? theme.border : 'transparent', color: theme.text, fontWeight: activeTab === 'lojas' ? 'bold' : 'normal' }}>
            <Layout size={18} /> Gestão de Lojas
          </button>
          <button onClick={() => setActiveTab('config')} style={{ padding: '12px', border: 'none', borderRadius: '8px', textAlign: 'left', cursor: 'pointer', display: 'flex', gap: '10px', background: activeTab === 'config' ? theme.border : 'transparent', color: theme.text, fontWeight: activeTab === 'config' ? 'bold' : 'normal' }}>
        ...
        <main style={{ flex: 1 }}>
          {activeTab === 'lojas' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              {/* FILTROS */}
              <div style={{ background: theme.card, borderRadius: '12px', border: `1px solid ${theme.border}`, padding: '15px', display: 'flex', gap: '15px', flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  <div key={local.id} style={{ background: theme.card, borderRadius: '12px', border: `1px solid ${theme.border}`, padding: '20px', borderLeft: isEditing ? `5px solid ${configForm.cor_primaria}` : (local.destaque ? '5px solid #f59e0b' : `1px solid ${theme.border}`) }}>
                    {isEditing ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        ...
                        <div style={{ background: theme.bg, padding: '15px', borderRadius: '8px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                          {TAGS_OFICIAIS.map(tag => (
                            <div key={tag.id} onClick={() => toggleTagForm(tag.id)} style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', padding: '6px 12px', borderRadius: '6px', background: editForm.tags?.includes(tag.id) ? tag.color : theme.card, color: editForm.tags?.includes(tag.id) ? 'white' : theme.textSec, fontSize: '12px', fontWeight: 'bold', border: `1px solid ${theme.border}` }}>
            ...
            <div style={{ background: theme.card, borderRadius: '12px', border: `1px solid ${theme.border}`, padding: '30px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                <div>
                  ...
                  <input value={configForm.titulo_pagina} onChange={e => setConfigForm({...configForm, titulo_pagina: e.target.value})} style={{ width: '100%', padding: '12px', border: `1px solid ${theme.border}`, borderRadius: '8px', background: theme.input, color: theme.text }} />
                </div>
                <div>
